...
function fetchalltaxdetail(param){
      var itemspan='';
      $.ajax({
          type: "POST",
          url: "../src/fetch_alltaxdetail.php",
          async:false,
          data:({
            transactionno:param
          }),
...
function editappenditemtable(param){
  var oitemname=editconnectitemname();
  var oitemtax=editconnectitemtax();
  var oitemunit=editconnectitemunit();
  var otaxdetail = fetchalltaxdetail(param);
  $.ajax({
      type: "GET",
      async:false,
      ...
      success: function(msg) {
        ...
        $("#ponumber").val(response[0]['PoNumber']);
        ...
        for(n=0;n<count;n++){
          ...
          t+='<select class="form-control" data-provide="selectpicker" data-live-search="true"  title="Choose a items" id="itemname'+(rows+1)+'" onchange="getiteminform('+(rows+1)+')" >';
          t+= oitemname;
          ...
          t +='</td>';
          t +='<td>';
          t +='<select class="form-control" data-provide="selectpicker" data-live-search="true"  title="Choose a Item Unit" id="itemunits'+(rows+1)+'" onchange="setitemunitinform('+(rows+1)+')" >';
          t += oitemunit;
          t +='</select>';
          // alert(t);
          ...
          t +='<td>';
          t +='<input type="hidden" id="hiddentax'+(rows+1)+'"/>';
          t +='<select class="form-control" data-provide="selectpicker" data-live-search="true"  title="Choose a TAX" id="tax'+(rows+1)+'" onchange="settaxintable('+(rows+1)+')">';  //
          t += oitemtax;
          t +='</select>';
      ...
      // / alert("#eitemname"+response[j]['itemunitval']);
        $("#itemname"+newval).val(response[j]['itemid']).trigger('change');
        $("#itemdetailid"+newval).val(response[j]['itemDetailId']);
        $("#hsn"+newval).val(response[j]['hsn']);
            ...
            var multotal = 1;
            switch(parseInt(response[j]['itemunitval'])){
              case 1:multotal =1;
                     break;
              case 2:multotal = response[j]['SubPacking'];
                     break;
              case 3:multotal = response[j]['PackingQty'];
                     break;
              default:multotal =1;
            }
            $("#unitsubpackingqty"+newval).val(response[j]['SubPacking']);
            $("#unitpackingqty"+newval).val(response[j]['PackingQty']);
            $("#unitremainqty"+newval).val(response[j]['TotalQty']);
            $("#hiddenqtyonhand"+newval).val(response[j]['Quantity']);
          $("#tax"+newval).val(response[j]['TaxPercent']).trigger('change');
          $("#qty"+newval).val(response[j]['qty']).trigger('change');
          $("#billingqty"+newval).val(response[j]['BillQty']);
          $("#rate"+newval).val(response[j]['rate']).trigger('change');
          $("#amount"+newval).val(response[j]['rate']*response[j]['qty']*multotal);
          $("#itemdiscount"+newval).val(discountval).trigger('change');

      }
      // alert(otaxdetail);
      var response1 = JSON.parse(otaxdetail);
      // alert(response1);
      var count1 = Object.keys(response1).length;
      // alert("Count1"+count1);
      var subtotal =0.00;
      for(var n=0;n<count1;n++){
        var taxvalue = parseFloat(response1[n]['TaxPercent']);
        var sum= parseFloat(response1[n]['Total_before_tax']);

        subtotal = subtotal +sum;
        // alert(subtotal);
        var cval= parseFloat(response1[n]['Tax']);
        var tsetid = (parseFloat(taxvalue).toFixed(3))*1000;
        var halftaxvalue = taxvalue/2;
        var hsetid = halftaxvalue.toFixed(3)*1000;
        // alert(halftaxvalue);
        var detailtax='';
        detailtax += "<tr id=cnewtax"+tsetid+">";
        detailtax += "<td><strong><span id=mcgst"+hsetid+"></span></strong></td>";
        detailtax += '<td><div class="row"><div class="col-sm-8"><input type="text" class="form-control form-control-sm" value="0.00"  id="CGST'+hsetid+'" readonly></div></div></td>';
        detailtax += "</tr>";
        detailtax += "<tr id=snewtax"+tsetid+">";
        detailtax += "<td><strong><span id=msgst"+hsetid+"></span></strong></td>";
        detailtax += '<td><div class="row"><div class="col-sm-8"><input type="text" class="form-control form-control-sm" value="0.00"  id="SGST'+hsetid+'" readonly></div></div></td>';
        detailtax += "</tr>";
        $("#gsttaxtable").append(detailtax);
        $("#mcgst"+hsetid).html("CGST @ "+halftaxvalue+"% on "+sum.toFixed(2));
        $("#msgst"+hsetid).html("SGST @ "+halftaxvalue+"% on "+sum.toFixed(2));
        $("#CGST"+hsetid).val(cval.toFixed(2));
        $("#SGST"+hsetid).val(cval.toFixed(2));
      }
      $('#subtotal').text(subtotal.toFixed(2));
      var total = parseFloat(response1[n-1]['FinalTotal']);
      $('#total').text(total.toFixed(2));
      $('#finaltotal').text(total.toFixed(2));
      $('#balancedue').text(total.toFixed(2));
      $("#invtemp").val("");
